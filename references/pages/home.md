...
| 元素 | 规格 |
|------|------|
| 背景 | Hero 使用首页展示色 `--studio-mint`；其余页面内容区域使用 `Bg`（`--bg-base`） |
| 标题 | 两行固定换行：`为AI工作流` / `创建的设计系统` |
| 正文 | 桌面固定两行完整展示，说明「文档是源头，demo 是验证，代码是最终交付」；小屏自然换行 |
| 前景 | Hero 标题与正文固定使用深色文字，不跟随站点主题反转 |
| CTA | 单按钮 `立即查看`，固定黑色背景与白色文字，指向 `#/docs/intro` |
