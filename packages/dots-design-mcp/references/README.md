...
页面级或 Pattern 级 demo 必须基于对应页面模板生成，不允许脱离页面模板另造展示基座。对话页能力必须复用真实对话页模板、状态栏、输入栏、消息流和消息组件；汇报说明、逻辑表、设计备注只能放在页面模板外侧。

回答 loading、长思考、快速回答、真实感 loading、tool call 来源展示、思考过程半层和来源半层统一沉淀在 [`frameworks/answer-loading.md`](frameworks/answer-loading.md)。页面文档只写页面基座和 demo 组织方式，组件文档只写可复用组件能力。
...
| 目录 | 内容 |
|------|------|
| `frameworks/` | 跨页面的交互框架（如 `framework-a.md` 聊天流、`answer-loading.md` 回答加载框架），多个页面共同依赖 |
...
- Framework：2 draft（framework-a 聊天流、answer-loading 回答加载框架）
