...
主站底部输入区固定为 393×98px：输入框区 60px、AI 辅助文案区 18px、Home Indicator 区 20px。输入框位于区域顶部，左右各 12px，尺寸 369×48px，底部留 12px；输入框内边距为上下 12px、左右 14px。整体使用 `#F5F5F5` 纵向透明渐变：`0% / 0%`、`25% / 80%`、`55% / 100%`，不使用 Background Blur。48px 输入框自身使用半透明 `#FCFCFC` 表面与 25px backdrop blur；该模糊只属于输入框，不影响底部渐变、辅助文案和 Home Indicator。
...
- 所有项目 Demo 必须通过共享 `ReportDemoCanvas` 渲染。该底座统一提供项目菜单、分享/关闭、背景拖动、高度自适应初始缩放、50%–150% 手动缩放和刷新复位；页面实现只能注入画板内容及可选的方案控制，不得私有复制拖动或缩放状态。
...
- 关闭按钮返回 `/docs` 主页。
- 画布右下角内缩 16px 放置 120px × 44px 的画布工具条，圆角 12px。左侧两个 44px 操作区使用 Figma 原始加号、减号图标，分别把手机画板与右侧控制器组成的完整内容组放大或缩小 5%。初始缩放根据屏幕高度自动计算：视口上下合计保留 112px 舒适空间，无论手机边框开关状态都固定以 912px 边框外形为缩放基准，按 5% 向下取整并限制在 50%–120%。边框开启和关闭只改变外观，不重算或跳变当前缩放。用户首次点击加减后进入手动模式，范围限制在 50%–150%，后续视口高度变化不覆盖手动值。点阵背景、左上项目入口、右上分享/关闭和右下工具条不参与缩放。
- 分享/关闭组合模块左侧展示 Figma「7月方案」节点 `2455:59150` 的「手机边框」开关；两者水平间距 40px，并按模块垂直中心线对齐。开关开启后，把 `src/assets/docs/iphone-bezel.png` 按 3x 素材的 1x 尺寸 `453×912px` 覆盖在现有 `393×852px` 手机画板外侧，四周各向外扩 30px；手机页面内容的尺寸、布局、裁切和缩放基准保持不变。关闭时恢复原有无设备边框预览。
...
- 方案面板只负责承载当前 demo 注入的方案控制，不在共享底座内写死业务状态。回答 Loading demo 保留 `胶囊 / 信息卡` 与「右侧图标动画」控制；问点点灵动展开 demo 注入 `方案 A / 方案 B` 的入口气泡控制。面板中的切换必须立即反映到手机画板。
...
- 当前项目 Demo 列表中的每个页面都必须显示缩放工具条。只有存在两个及以上方案的页面显示第三个方案入口；其他页面使用 88px 的纯缩放工具条。刷新页面后恢复自动缩放模式。
