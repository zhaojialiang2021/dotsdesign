...
页面中的接入步骤使用扁平章节结构：步骤本身不包卡片，代码块或命令块是唯一的内容承载面，步骤之间只用留白分隔，避免步骤卡再嵌套代码卡。

推荐接入路径在桌面端使用三张等宽、等高卡片横排，卡片顶部和底部对齐，高度以同一行最高内容为准；窄屏回落为单列。
...
成功不是看配置贴没贴上，而是看 AI 能不能真的调用 Dots。按下面顺序验收：

1. 保存 MCP 配置后，完全退出并重启 AI 工具。
2. 在 AI 工具的 MCP / Tools / Integrations 列表里找 `dots-design`。能看到它，说明配置文件已经被工具读到了。
3. 新开一个对话，让 AI 调用 `get_demo_workflow`。不要只问“你知道 Dots 吗”，要明确让它调用工具。
4. 如果返回 demo 制作流程、页面基座规则、文档读取顺序或验收清单，就说明接入成功。
5. 再让 AI 搜索“回答 loading”“页面模板”或组件名。能找到对应规范，说明后续 demo 能按设计系统工作。
...
3. 如果提示找不到包，在终端运行 `npx -y dots-design-mcp`。这里也失败，先处理 npm 登录、网络或包名问题。
...
| 提示找不到 `dots-design-mcp` | 先在终端运行 `npx -y dots-design-mcp`；如果这里也失败，检查 npm 登录、网络、包名、命令和 `-y` 参数。 |
